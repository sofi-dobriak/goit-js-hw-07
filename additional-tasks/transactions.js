...
function transactionTamplate(transactions) {
    const isPositive = transactions.amount > 500;
    const transactionType = isPositive ? 'deposit' : 'withdraw';

    return `<tr class='table-item ${transactionType}'>
                <th>${transactions.id}</th>
                <th>${transactions.amount}</th>
                <th>${transactions.date}</th>
                <th>${transactions.business}</th>
                <th>${transactions.type}</th>
                <th>${transactions.name}</th>
                <th>${transactions.account}</th>
            </tr>`;
}

function transactionsTamplate(array) {
    return array.map(transactionTamplate).join('');
}

const markup = transactionsTamplate(transactionHistory);

const tableElem = document.querySelector('.js-transaction-table');
const itemElem = tableElem.lastElementChild;
itemElem.insertAdjacentHTML('afterbegin', markup);
